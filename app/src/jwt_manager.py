...
class JWTManager:
    def __init__(self, secret_key: str, expiration_seconds: Optional[int] = 900):
    ...
    def create_token(self, agent_id: int) -> str:
        ...
        if self.expiration_seconds is not None:
            payload["exp"] = now + timedelta(seconds=self.expiration_seconds)
    ...
    def verify_token_no_update(self, token: str) -> JWTSessionData:
        ...
        try:
            ...
            if token in self.sessions:
                ...
                updated_at_str = payload.get("updated_at", now.isoformat())
                session_data = JWTSessionData(
            ...
            if self.expiration_seconds is not None:
                time_since_activity = now - session_data.updated_at
