...
from src.dochandler.src.utils.data_formatter import DataFormatter
from src.cgcore.utils.data_type import detect_datatype
import asyncio
...
class Loader:
    ...
    async def extr_load(self, source: str, metadata: Optional[Dict] = {}):
        ...
        for i, chunk_id in enumerate(chunks["ids"]):
            content = chunks["documents"][i]
            question_response = await self.llm.judge(content)
