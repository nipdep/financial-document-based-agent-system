...
class ExTrRAGV2(BaseRAG):
    def __init__(self, llm, db, embedder, cluster:DensityBasedCluster, memory="none", history=False):
        self.llm = llm
        self.db = db
        ...
        if ruling.decision:
            ...
            if len(extra_questions) > 0:
                for question_prompt in extra_questions:
                    related_docs = self.retriever.extr_cluster_retrieve(question_prompt, cluster=self.cluster)
                    context.extend(related_docs)
    ...
    def add_document(self, source, metadata={}):
        self.loader.extr_cluster_load(source, metadata, cluster=self.cluster)
