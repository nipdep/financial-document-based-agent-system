    ...
    def load_data(content):
        ...
        try:
            ...
            raise Exception(
                ...
                Do `pip install --upgrade 'chatgenie[json]`"
            )
        ...
        if not isinstance(content, str):
            print(
                f"Invaid content input. Provide the correct path to the json file saved locally in {content}")
        ...
        if os.path.isfile(content):
            ...
            raise ValueError(
                f"Invalid content to load json data from: {content}")

        docs = loader.load_data(json_data)
        for doc in docs:
            doc_content = doc.text
            data.append({"content": doc_content,
                        "meta_data": {"url": content}})
            data_content.append(doc_content)
        doc_id = hashlib.sha256(
            (content + ", ".join(data_content)).encode()).hexdigest()
