...
try:
    ...
    raise ImportError(
        'DocsSite requires extra dependencies. Install with `pip install --upgrade "chatgenie[dataloaders]"`'
    ) from None
...
class DocsSiteLoader(BaseLoader):
    ...
    def _get_child_links_recursive(self, url):
        ...
        if response.status_code != 200:
            logging.info(
                f"Failed to fetch the website: {response.status_code}")
            return

        soup = BeautifulSoup(response.text, "html.parser")
        all_links = [link.get("href") for link in soup.find_all("a")]

        child_links = [link for link in all_links if link and link.startswith(
            current_path) and link != current_path]
    ...
    def _get_all_urls(self, url):
        ...
        urls = [link for link in self.visited_links if urlparse(
    ...
    def load_data(self, url):
        ...
        doc_id = hashlib.sha256(
            (" ".join(all_urls) + url).encode()).hexdigest()
        return {
            "doc_id": doc_id,
            "data": output,
        }
