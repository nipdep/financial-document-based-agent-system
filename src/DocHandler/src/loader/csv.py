...
class CsvLoader(BaseLoader):
    ...
    def _detect_delimiter(first_line):
        delimiters = [",", "\t", ";", "|"]
        counts = {delimiter: first_line.count(
    ...
    def _get_file_content(content):
        ...
        if url.scheme in ["http", "https"]:
            ...
            path = url.path
            # Open the file using the path from the URI
            return open(path, newline="")
        else:
            # Treat content as a regular file path
    ...
    def load_data(content):
        ...
        with CsvLoader._get_file_content(content) as file:
            ...
            for i, row in enumerate(reader):
                ...
                lines.append(line)
                result.append({"content": line, "meta_data": {
                              "url": content, "row": i + 1}})
        doc_id = hashlib.sha256(
            (content + " ".join(lines)).encode()).hexdigest()
